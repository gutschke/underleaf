...
>
> This folder holds the DM-only material for *Unattended Baggage*. Files here describe twists, hidden mechanics, and the spoiler-bearing twin of every public scene. If you are or might become a player in this campaign, close this folder now.
>
> In a published Underleaf fork, `quire encrypt-dm` encrypts this folder at rest. In the upstream development checkout, files are kept in plaintext so the design can be iterated on — this warning is the only barrier.

# Contents

This episode is the entire campaign's foundation moment. The DM should read every file in this folder before running the session.
